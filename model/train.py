...
def run_validation(model, validation_ds, tokenizer_src, tokenizer_tgt, max_len, device, print_msg, global_step, writer, num_examples=2):
    ...
    with torch.no_grad():
        for batch in validation_ds:
            ...
            model_out = beam_search_decode(model, 2, encoder_input, encoder_mask, tokenizer_src, tokenizer_tgt, max_len, device)
...
def train_model(config):
    ...
    # Save the model at the end of every training session
    model_filename = get_weights_file_path(config, f"0")
    torch.save({
        'epoch': 0,
